...
validate_finemap_data <- function(data) {
	...
	if (anyNA(data$POS) || any(!is.finite(as.numeric(data$POS))) || any(as.numeric(data$POS) < 1)) {
		stop("POS must contain positive numeric base-pair positions.", call. = FALSE)
	}
	if (anyNA(data$SNP) || any(!nzchar(trimws(as.character(data$SNP))))) {
		stop("SNP must contain a non-empty variant identifier for every row.", call. = FALSE)
	}
	if (anyNA(data$PIP) || any(!is.finite(as.numeric(data$PIP))) || any(as.numeric(data$PIP) < 0 | as.numeric(data$PIP) > 1)) {
...
read_finemap_csv <- function(path) {
	...
	data <- tryCatch(
		...
		error = function(error) stop("Could not read CSV: ", error$message, call. = FALSE)
	)
...
filter_finemap_data <- function(data, position_range) {
	...
	data[data$POS >= position_range[1] & data$POS <= position_range[2], , drop = FALSE]
}

top_finemap_variants <- function(data, top_n) {
	data[order(-data$PIP, data$POS), , drop = FALSE][seq_len(min(top_n, nrow(data))), , drop = FALSE]
}
