...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      ...
      #sidebar_wrapper > .row > div:first-child {
        transition: width 0.15s ease;
      }
      #sidebar_wrapper.collapsed > .row > div:first-child {
        display: none;
      }
      #sidebar_wrapper.collapsed > .row > div:nth-child(2) {
        width: 100%;
        flex: 0 0 100%;
        max-width: 100%;
      }
  ...
  titlePanel(
    tagList(
      actionLink(
        "toggle_sidebar",
        label = icon("bars"), class = "sidebar-toggle-btn"
      ),
  ...
  div(
    ...
    navlistPanel(
      ...
      tabPanel(
        ...
        h4("About this app"),
        p(paste0(
          "This app explores statistical fine-mapping results for ",
          "chromosome 22, highlighting variants with high posterior ",
          "inclusion probability (PIP) and their credible sets."
        ))
      ),
      tabPanel(
        "Settings",
...
server <- function(input, output, session) {
  ...
  data_state <- reactive({
    ...
    if (input$finemap_file$size > max_upload_bytes) {
      return(list(
        data = NULL,
        error = "The uploaded file is larger than 100 MB.",
        source = "uploaded CSV"
      ))
    }
    tryCatch(
      list(
        data = read_finemap_csv(input$finemap_file$datapath),
        error = NULL,
        source = input$finemap_file$name
      ),
  ...
  output$pip_plot <- renderPlot({
    plot_finemap_pip(
      current_data(),
      input$position_range,
      input$pip_threshold,
      input$credible_set_threshold
    )
  })
  ...
  output$variant_table <- renderTable(
    {
      ...
      validate(need(
        nrow(data) > 0,
        "No variants are in the selected position range."
      ))
  ...
  output$data_status <- renderPrint({
    ...
    cat(
      "Total PIP in view:",
      formatC(sum(data$PIP), format = "f", digits = 4),
      "\n"
    )
  })
  ...
  output$download_variants <- downloadHandler(
    filename = function() {
      paste0("chr22_visible_finemapping_", Sys.Date(), ".csv")
    },
